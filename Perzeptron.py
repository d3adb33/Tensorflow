...
class Perceptron():
    ...
    def train(self, x, y):
        ...
        for epoch in range(self.epochs):
            ...
            if y_hat != y_i:
                error += 1
                self.update_weight(x_i, y_i, y_hat)
        print("Train Error: ", error /y.shape[0])

    def test(self, x, y):
        y_pred = np.array([self.predict(x_i) for x_i in x])
        acc = sum(1 for y_p, y_i in zip(y_pred, y) if y_p == y_i) / y.shape[0] #prüfung und bei richtigem Ergebnis wird es gespeichert
        #return acc #genauigkeit
        print("Test ACC: ", acc)
        
    def update_weight(self, x, y, y_hat):
        for i in range(self.w.shape[0]):
            #delta w_i = lr * (y - y_hat) + x[i]
